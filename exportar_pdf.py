...
def autodetect_secondary_cover_image(primary: Optional[str] = None) -> Optional[str]:
    """
    Busca preferentemente 'Portada 2.jpeg'. Si no está, intenta con variantes y patrones comunes:
    subportada / portada2 / imagen_extra / logo2 / banner / 'Portada 2.*' / 'portada 2.*'
    """
    root = Path.cwd()
    exts = {".png", ".jpg", ".jpeg", ".webp"}

    # 1) Prioridad: exactamente "Portada 2.jpeg" (en raíz o /assets o subcarpetas)
    prefer = []
    for base in [root, root / "assets"]:
        prefer += list(base.glob("Portada 2.jpeg"))
    if not prefer:
        prefer += list(root.rglob("Portada 2.jpeg"))
    prefer = [p for p in prefer if p.suffix.lower() in exts]
    if prefer:
        if primary:
            try:
                prefer = [p for p in prefer if Path(primary).resolve() != p.resolve()]
            except Exception:
                pass
        if prefer:
            return str(prefer[0])

    # 2) Alternativas directas: "Portada 2.*" y "portada 2.*"
    alt = []
    for base in [root, root / "assets"]:
        alt += list(base.glob("Portada 2.*"))
        alt += list(base.glob("portada 2.*"))
    if not alt:
        alt += list(root.rglob("Portada 2.*"))
        alt += list(root.rglob("portada 2.*"))

    # 3) Patrones genéricos
    patterns = ["*subportada.*", "*portada2.*", "*imagen_extra.*", "*logo2.*", "*banner.*"]
    for base in [root, root / "assets"]:
        for p in patterns:
            alt += list(base.glob(p))
    if not alt:
        for p in patterns:
            alt += list(root.rglob(p))

    alt = [p for p in alt if p.suffix.lower() in exts]
    if primary:
        try:
            alt = [p for p in alt if Path(primary).resolve() != p.resolve()]
        except Exception:
            pass
...
# ========= PDF helpers =========
def portada(c: canvas.Canvas, image_path: Optional[str], image_path2: Optional[str], canton: str):
    y_top = A4[1] - 2.5*cm

    # Imagen 1 (igual que antes)
    if image_path:
        try:
    ...
    # Imagen 2 (nueva) — centrada debajo de la primera y encima del título
    if image_path2:
        try:
            img2 = ImageReader(image_path2)
            iw2, ih2 = img2.getSize()
            # Un poco más pequeña para no empujar demasiado el título
            scale2 = min((A4[0]-5*cm)/iw2, (5.5*cm)/ih2)
            w2, h2 = iw2*scale2, ih2*scale2
            x2 = (A4[0]-w2)/2; y2 = y_top - h2
            c.drawImage(img2, x2, y2, width=w2, height=h2, preserveAspectRatio=True, mask='auto')
            y_top = y2 - 0.8*cm
        except Exception:
            pass
...
def build_pdf_grouped_by_problem(rows: pd.DataFrame, image_path: Optional[str], image_path2: Optional[str], canton: str) -> bytes:
    buf = io.BytesIO(); c = canvas.Canvas(buf, pagesize=A4)
    portada(c, image_path, image_path2, canton)
...
with st.sidebar:
    ...
    cover2_path = autodetect_secondary_cover_image(cover_path)
    if cover_path:
    ...
    if cover2_path:
        st.info(f"Segunda imagen de portada: `{Path(cover2_path).as_posix()}`")
...
if st.button("Generar PDF editable"):
    pdf = build_pdf_grouped_by_problem(regs_muni, cover_path, cover2_path, canton_name)
